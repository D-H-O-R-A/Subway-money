...
if ($result_soma_percas) {
    ...
    if ($row) { 
        ...
        if ($result_verificar_ggr) {
            ...
            if ($result_obter_taxa) {
                ...
                if ($result_obter_total_percas) {
                    ...
                    $sql_atualizar_ggr = "UPDATE ggr SET total_percas = ?";
                    $stmt = $conn->prepare($sql_atualizar_ggr);
                    if ($stmt === false) {
                        die("Erro ao preparar a consulta: " . $conn->error . "\n");
                    }

                    $stmt->bind_param("i", $total_percas); // Substitua "i" por "s", "d", etc., conforme o tipo de $total_percas
                    if ($stmt->execute()) {
                        echo "Valor total de perdas atualizado na tabela 'ggr' com sucesso!\n";
                    } else {
                        echo "Erro ao atualizar o valor total de perdas na tabela 'ggr': " . $stmt->error . "\n";
                    }
                    ...
                    if ($result_calcula_diferenca) {
                        ...
                        $sql_atualizar_debito_ggr = "UPDATE ggr SET debito_ggr = ?";
                        $stmt = $conn->prepare($sql_atualizar_debito_ggr);

                        if ($stmt === false) {
                            die("Erro ao preparar a consulta: " . $conn->error . "\n");
                        }

                        // Vincular parâmetros à declaração preparada (ajuste o tipo de parâmetro conforme necessário)
                        $stmt->bind_param("i", $resultado); // Substitua "i" por "s", "d", etc., dependendo do tipo de $resultado

                        // Executar a declaração
                        if ($stmt->execute()) {
                            echo "Valor da diferença adicionado à coluna 'debito_ggr' com sucesso!\n";
                        } else {
                            echo "Erro ao atualizar o valor da coluna 'debito_ggr': " . $stmt->error . "\n";
                        }
        ...
        echo "Nenhum dado encontrado para total de perdas.\n";
    }    
