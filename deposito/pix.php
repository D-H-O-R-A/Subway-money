...
if (!empty($externalReference) && !empty($email) && !empty($valor)) {
    try {
        ...
        $conn = new PDO("mysql:host=193.203.175.35;dbname={config['db_name']}", config['db_user'], config['db_pass']);
...
async function c() {
    ...
    while (new Date().getTime() < now + interval) {
        ...
        const url = 'http://subwayoriginal.online/deposito/consultarpagamento.php?token=' + token;
        await fetch(url)
            .then((resp) => resp.json())
            .then(function ({status}) {
                console.log(status)
                if (status === 'PAID_OUT') {
                    window.location.href = 'https://subwayoriginal.online/deposito/confirmacao.html';
                }
            })
